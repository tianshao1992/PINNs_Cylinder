...
from basic_model_pdpd import gradients, DeepModel_single
import visual_data
...
class Net(DeepModel_single):
    ...
    def equation(self, inn_var, out_var):
        ...
        d2udx2 = gradients(dudx, inn_var, create=False)[:, 0:1]
        d2udy2 = gradients(dudy, inn_var, create=False)[:, 1:2]
        d2vdx2 = gradients(dvdx, inn_var, create=False)[:, 0:1]
        d2vdy2 = gradients(dvdy, inn_var, create=False)[:, 1:2]
...
def train(inn_var, BCs, ICs, out_true, model, Loss, optimizer, scheduler, log_loss, opts):
    ...
    inn_var = paddle.concat((inn_var[inn], inn_var[BC_in], inn_var[BC_out], inn_var[BC_wall], inn_var[BC_meas], inn_var[IC_0]), axis=0)
    out_true = paddle.concat((out_true[inn], out_true[BC_in], out_true[BC_out], out_true[BC_wall], out_true[BC_meas], out_true[IC_0]), axis=0)
...
if __name__ == '__main__':
    ...
    work_name = 'NS-cylinder-2d-t_pdpd_' + points_name
